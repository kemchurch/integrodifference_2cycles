...
N_manif_compute = [30,20];  # Number of Taylor coefficients for the computation of the manifold parameterization
...
#=
component(a,1)[(0,0)] = equilibrium₂[1]
component(a,2)[(0,0)] = equilibrium₂[2]
component(a,3)[(0,0)] = equilibrium₂[3]
component(a,4)[(0,0)] = equilibrium₂[4]

component(a,1)[(1,0)] = ξ₃[1]
component(a,2)[(1,0)] = ξ₃[2]
component(a,3)[(1,0)] = ξ₃[3]
component(a,4)[(1,0)] = ξ₃[4]

component(a,1)[(0,1)] = ξ₄[1]
component(a,2)[(0,1)] = ξ₄[2]
component(a,3)[(0,1)] = ξ₄[3]
component(a,4)[(0,1)] = ξ₄[4]
=#

a, = newton!((F, DF, a) -> (F_manif!(F, a, N_manif_compute, σ, r, equilibrium₂, λ₃, λ₄, ξ₃, ξ₄), DF_manif!(DF, a, N_manif_compute, σ, r, equilibrium₂, λ₃, λ₄, ξ₃, ξ₄)), a, tol = 1e-14)
...
#=
println(component(X,1))
println(component(component(X,2),1))
println(component(component(X,2),2))
println(component(component(X,3),1)
println(component(component(X,3),2))
println(component(component(X,3),3))
println(component(component(X,3),4))
=#

S_orbit_compute = ParameterSpace() × ParameterSpace()^2 × Chebyshev(N_cheb_compute)^4
X = zeros(S_orbit_compute)

X.coefficients[:] = load_object(raw"ricker\orbit.jld2")
println(X)

X, = newton!((F, DF, X) -> (F_orbit!(F, X, a, N_cheb_compute, σ, r), DF_orbit!(DF, X, a, N_cheb_compute, σ, r)), X, tol = 1e-14)
...
connecting_orbit_data, connecting_orbit_time = orbit_sol_to_data(X, 100)
...
println(N(0.0, λ₃, λ₄, a, X))
# Plotting

# Using GLMakie
figRicker = Figure()

ODEax = Axis3(figRicker[1,1],
...
Colorbar(figRicker[1,2], limits = (-4,4), label = L"x_4", labelsize = 20)
axislegend()


IDEax = Axis(figRicker[1,3],
...
display(GLMakie.Screen(),figRicker)
